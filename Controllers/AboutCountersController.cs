...
namespace ApexWebAPI.Controllers
{
    ...
    public class AboutCountersController : ControllerBase
    {
        private readonly ApexDbContext _context;
        private readonly IMapper _mapper;
        private readonly IStringLocalizer<AboutCountersController> _localizer;

        public AboutCountersController(ApexDbContext context, IMapper mapper, IStringLocalizer<AboutCountersController> localizer)
        {
            _context = context;
            _mapper = mapper;
        ...
        {
            var item = await _context.AboutCounters!
            ...
            var dto = _mapper.Map<GetByIdAboutCounterDto>(item);
        ...
        public async Task<IActionResult> Create([FromRoute] string lang, [FromBody] CreateAboutCounterDto dto)
        {
            var existing = await _context.AboutCounters!
                .Include(c => c.Translations)
                .FirstOrDefaultAsync();

            if (existing != null)
                _context.AboutCounters.Remove(existing);

            var item = new AboutCounter
            {
                Count1 = dto.Count1,
                Count2 = dto.Count2,
                Count3 = dto.Count3,
                ...
                Translations = new List<AboutCounterTranslation>
                {
            ...
            await _context.AboutCounters!.AddAsync(item);
        ...
        [ProducesResponseType(404)]
        public async Task<IActionResult> Update([FromRoute] string lang, [FromBody] UpdateAboutCounterDto dto)
        {
            var item = await _context.AboutCounters!
                .Include(c => c.Translations)
            ...
            foreach (var (language, (t1, t2, t3, t4)) in translations)
            {
                ...
                {
                    item.Translations!.Add(new AboutCounterTranslation
                    {
                        Language = language,
                        Text1 = t1,
                        Text2 = t2,
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            _context.AboutCounters.Remove(item);
            await _context.SaveChangesAsync();
